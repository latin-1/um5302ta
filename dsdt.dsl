 ...
 *
 * Disassembly of dsdt.dat, Fri Mar 10 00:10:09 2023
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000E2D8 (58072)
 *     Revision         0x02
 *     Checksum         0xFB
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x01072009)
{
    ...
    Scope (_SB.PCI0.SBRG)
    {
        ...
        Device (EC0)
        {
            ...
            {
                Name (ATCS, Buffer (0x39){})
                CreateWordField (ATCS, Zero, SSZE)
                CreateByteField (ATCS, 0x02, FPPT)
                CreateDWordField (ATCS, 0x03, SMU1)
                CreateByteField (ATCS, 0x07, SPPT)
                CreateDWordField (ATCS, 0x08, SMU2)
                CreateByteField (ATCS, 0x0C, SPTT)
                CreateDWordField (ATCS, 0x0D, SMU3)
                CreateByteField (ATCS, 0x11, THLT)
                CreateDWordField (ATCS, 0x12, SMU4)
                CreateByteField (ATCS, 0x16, PDRT)
                CreateDWordField (ATCS, 0x17, SMU5)
                CreateByteField (ATCS, 0x1B, STLA)
                CreateDWordField (ATCS, 0x1C, SMU6)
                CreateByteField (ATCS, 0x20, SMLT)
                CreateDWordField (ATCS, 0x21, SMU7)
                CreateByteField (ATCS, 0x25, TDCV)
                CreateDWordField (ATCS, 0x26, SMU8)
                CreateByteField (ATCS, 0x2A, EDCV)
                CreateDWordField (ATCS, 0x2B, SMU9)
                CreateByteField (ATCS, 0x2F, TDCS)
                CreateDWordField (ATCS, 0x30, SMUA)
                CreateByteField (ATCS, 0x34, P3TW)
                CreateDWordField (ATCS, 0x35, SMUB)
                SSZE = 0x39
                FPPT = 0x06
                SPPT = 0x07
                SPTT = 0x08
                ...
                TDCS = 0x0E
                P3TW = 0x32
                Name (ATC2, Buffer (0x11){})
                CreateWordField (ATC2, Zero, SSZ2)
                CreateByteField (ATC2, 0x02, FPP2)
                CreateDWordField (ATC2, 0x03, SM21)
                CreateByteField (ATC2, 0x07, SPP2)
                CreateDWordField (ATC2, 0x08, SM22)
                CreateByteField (ATC2, 0x0C, SML2)
                CreateDWordField (ATC2, 0x0D, SM23)
                SSZ2 = 0x11
                FPP2 = 0x06
                SPP2 = 0x07
                SML2 = 0x2E
            ...
            Scope (\_SB.PCI0.SBRG.EC0)
            {
                ...
                Device (BATT)
                {
                    ...
                    Method (_BST, 0, NotSerialized)  // _BST: Battery Status
                    {
                        ...
                        If ((ECWR & 0x08))
                        {
                            VAL1 |= One /* \_SB_.PCI0.SBRG.EC0_.BATT._BST.VAL1 */
                        }

                        If ((ECWR & 0x04))
                        {
                            VAL1 |= 0x02 /* \_SB_.PCI0.SBRG.EC0_.BATT._BST.VAL1 */
                        }

                        If ((ECWR & 0x20))
                        {
                            VAL1 |= 0x04 /* \_SB_.PCI0.SBRG.EC0_.BATT._BST.VAL1 */
                        }

                        If ((HBDA < 0x64))
                        {
                            VAL1 |= 0x08 /* \_SB_.PCI0.SBRG.EC0_.BATT._BST.VAL1 */
                        }

                        PKG1 [Zero] = VAL1 /* \_SB_.PCI0.SBRG.EC0_.BATT._BST.VAL1 */
                        If ((ECWR & One))
                        {
